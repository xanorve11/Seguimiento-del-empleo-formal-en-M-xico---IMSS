  ...
  # Objetivo: conformar las bases de empleo del IMSS por
...
system.time({
  ...
  cargar_datos <- function(archivo) {
    ...
    datos <- datos %>%
      # 1. Convertir a texto para evitar conflicto de tipos
      mutate(
      ...
      # 3. Etiquetar categorías (incluyendo NA)
      mutate(
        categoria = case_when(
          # ---- Sector económico ----
      ...
      # 4. Reordenar y agregar
      select(year, month, date, variable, categoria, ta, sexo) %>%
      group_by(year, month, date, variable, categoria, sexo) %>%
      summarise(ta = sum(ta, na.rm = TRUE), .groups = "drop") %>%
      arrange(date, variable, categoria, sexo)
  ...
  archivo_salida <- glue("outputs/imss_desagregado.rds")
